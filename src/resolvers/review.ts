...
export class ReviewResolver {
  ...
  async getReviewsForUser (
    ...
    try {
      ...
      throw new Error (`ReviewResolver.getReviewsForUser errored. Error-Msg.: ${error}`);      
    }
  }
  ...
  async getLastReviewsForUser (@Arg('query') query : ReviewQuery): Promise<Review[]> {
    try {
      ...
      throw new Error (`ReviewResolver.getLastReviewsForUser errored. Error-Msg.: ${error}`);
    }
  }
  ...
  async getSubmittedReview (@Arg('query') query : ReviewQuery) {
    try {
      const user = await GigUser.findOne(query.userId);
      const submittedReview = await Review.findOne({where: {user: user, fromUserId: query.fromUserId, skillId: query.skillId} });
      if (submittedReview) {
        return true;
      } else {
        return false;
      ...
      throw new Error (`ReviewResolver.getSubmittedReview errored. Error-Msg.: ${error}`);
    }
  }
  ...
  async getAvgRatingForSkill (@Arg('query') query : ReviewQuery) {
    try {
      ...
      throw new Error (`ReviewResolver.getAvgRatingForSkill errored. Error-Msg.: ${error}`);
    }
  }
      ...
      if (user) {
        ...
        if (input.skillId) {
          ...
          if (skill && relation) {
            ...
            throw `ReviewResolver.addReview errored. Skill or Skill-User Relation doesn't exist for user ${input.userId} and skill ${skill}`;
          }
        }
        ...
        throw `ReviewResolver.addReview errored. Couldn't find user ${input.userId}`;
      }
    } catch (error) {
      throw new Error (`ReviewResolver.addReview errored. Error-Msg.: ${error}`);
    }
  }
